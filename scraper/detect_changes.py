...
def generate_change_summary() -> Dict[str, Any]:
    ...
    if blog["count"] > 0:
        summary_lines.append(f"📝 Blog Posts: {blog['count']} new article{'s' if blog['count'] != 1 else ''}")
    ...
    if videos["count"] > 0:
        summary_lines.append(f"🎥 Videos: {videos['count']} new video{'s' if videos['count'] != 1 else ''}")
...
def get_whats_new(days: int = 7) -> Dict[str, Any]:
    ...
    recent_blog_posts = [
        post for post in blog["new_posts"]
        if post.get("date", "") >= cutoff_str
    ]
    
    # Filter videos by date
    recent_videos = [
        video for video in videos["new_videos"]
        if video.get("date", "") >= cutoff_str
    ]
