...
try:
    ...
    from scraper.utils import safe_write_file, ensure_dir
except ImportError:
...
def fetch_github_blog() -> List[Dict]:
    ...
    try:
        feed = feedparser.parse(GITHUB_BLOG_FEED)
...
def fetch_github_changelog() -> List[Dict]:
    ...
    try:
        feed = feedparser.parse(GITHUB_CHANGELOG_FEED)
...
def filter_copilot_content(entries: List[Dict]) -> List[Dict]:
    ...
    logger.info(f"Filtered to {len(filtered)} Copilot-related posts from {len(entries)} total entries")
    return filtered
