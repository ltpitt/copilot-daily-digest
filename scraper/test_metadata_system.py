...
def test_metadata_loading():
    ...
    try:
        ...
        # Check structure
        required_keys = ['version', 'last_updated', 'content_hashes', 'video_ids', 
                        'blog_urls', 'doc_versions', 'stats']
...
def test_duplicate_detection():
    ...
    try:
        ...
        if metadata['stats']['total_blog_posts'] != 1:
            results.add_fail("Blog stats", f"Expected 1, got {metadata['stats']['total_blog_posts']}")
            return
...
def test_change_detection():
    ...
    try:
        ...
        if stored_hash != expected_hash:
            results.add_fail("Hash verification", f"Stored hash doesn't match: {stored_hash} != {expected_hash}")
            return
...
def test_changes_summary():
    ...
    try:
        ...
        # Check required keys
        required_keys = ['last_updated', 'total_docs', 'total_videos', 
                        'total_blog_posts', 'last_successful_scrape']
        for key in required_keys:
