...
from scraper.metadata import add_video_id
from scraper.utils import ensure_directory, now_iso, parse_iso, safe_write_file
...
def enrich_with_api_data(videos: List[dict]) -> List[dict]:
    ...
    try:
        ...
        for i in range(0, len(video_ids), batch_size):
            ...
            videos_response = (
                youtube.videos()
                .list(id=",".join(batch), part="contentDetails,statistics")
                .execute()
            )
...
def filter_by_age(videos: List[dict], max_days: int) -> List[dict]:
    ...
    logger.info(
        f"Filtered by age ({max_days} days): {len(filtered)} videos from {len(videos)} total"
    )
    return filtered
...
Examples:
  ...
  python scraper/fetch_youtube.py --max-age-days 14 --require-keywords --dry-run
    ...
    parser.add_argument(
        "--max-age-days", type=int, help="Maximum age of videos in days (overrides config)"
    )
...
def main():
    ...
    if args.require_keywords:
        ...
        logger.info(
            f"Keyword filtering from config: {'enabled' if require_keywords else 'disabled'}"
        )
    ...
    if new_count > 0:
        ...
        sorted_videos = sorted(all_videos, key=lambda x: x.get("published", ""), reverse=True)
