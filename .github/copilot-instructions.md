...
GitHub Action (daily-agent.yml)
    ...
    │   └── generate_content.py → content/*.md (auto-generated)
    │
    ▼
YOU (This Agent) - same environment, fresh data AND content ready
    │
    ├─► Phase 1: Verify Data & Content
    ├─► Phase 2: Review & Fix Content Quality
...
Check that required data and content exists:
...
├── WHATS-NEW.md          # Auto-generated
├── DEPRECATIONS.md       # Auto-generated
├── CHANGELOG.md          # Auto-generated
├── VIDEOS.md             # Auto-generated
├── TRAININGS.md          # Auto-generated
├── EXPERIMENTAL.md       # Auto-generated
├── GETTING-STARTED.md    # Auto-generated
...
**If content files are missing or appear empty**: This indicates a setup step failure. Report the issue.

### Phase 2: Review & Fix Content Quality

The content files have been auto-generated by `scripts/generate_content.py`. Your job is to **review and fix quality issues**, not regenerate from scratch.

**Your responsibilities**:

1. **Review auto-generated content** for quality issues:
   - Check WHATS-NEW.md for proper journalistic summaries (no HTML artifacts, no RSS boilerplate)
   - Verify dates are complete (e.g., "Feb 13, 2026" not "Feb 2026")
   - Ensure summaries are 2-3 complete sentences, readable and informative
   - Check that experimental content has proper disclaimers

2. **Fix formatting issues**:
   - Replace HTML artifacts with clean text
   - Remove RSS boilerplate like "The post X appeared first on..."
   - Ensure proper markdown formatting
   - Fix incomplete sentences or truncated text

3. **Improve summaries when needed**:
   - If auto-generated summary is poor quality (HTML lists, fragments), create a proper journalistic summary
   - Use the blog post's summary field when available (it's usually cleaner than the content field)
   - Focus on practical value: what developers will learn or can do
   - Use active voice and engaging language

**DO NOT**:
- Regenerate content files from scratch (they're already generated)
- Remove or skip articles that were properly included
- Change the overall structure or organization

**Files to review** (all in `content/` directory):
- README.md - Navigation hub with stats
- WHATS-NEW.md - Last 30 days updates (PRIORITY: check summaries here)
- DEPRECATIONS.md - High-impact changes (deprecations, removals, breaking changes)
- CHANGELOG.md - Complete timeline
- VIDEOS.md - Categorized video library
- TRAININGS.md - Courses & certifications
- EXPERIMENTAL.md - GitHub Next projects (check disclaimers)
...
### content/DEPRECATIONS.md
**Purpose**: High-impact "action required" view — deprecations, retirements/removals, breaking changes, and security or policy/billing updates

**How it's generated**: Auto-derived by `generate_content.py` from `CHANGELOG.md`. It classifies **Blog** entries by title keyword (see `IMPACT_CATEGORIES` in the script) into: 🚫 Deprecation, 🗑️ Retirement/Removal, 💥 Breaking change, 🔒 Security, 📋 Policy/Billing. Videos are excluded so news round-ups are not misflagged.

**Structure**:
- Intro + legend of categories
- Organized by month (newest first), same date format as CHANGELOG
- Each entry: `- **Date** - {emoji} **Category** - [Title](url)`

**Review notes**:
- This page is derived from CHANGELOG — fix a wrong classification by adjusting the item's title/data or the keyword lists, not by hand-editing the generated file
- A short **⚠️ Action Required** callout at the top of WHATS-NEW.md mirrors the last-30-day subset; keep the two consistent
...
- [ ] DEPRECATIONS.md reflects CHANGELOG and WHATS-NEW "Action Required" callout is consistent
