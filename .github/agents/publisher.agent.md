...
### 🗓️ Blog Post Dates
- `data/blog/url_dates.json` - Maps blog URLs to publish dates (YYYY-MM-DD)
- **CRITICAL**: Use this file to get dates for blog posts
- Generated by `scraper/enrich_blog_dates.py`
- Contains dates for both changelog URLs and regular blog posts

## 🚨 CRITICAL: Date Requirements for WHATS-NEW.md

**All dates in WHATS-NEW.md MUST be complete and articles MUST be sorted chronologically.**
...
4. **Format dates for display**:
   - ✅ CORRECT: "Dec 8, 2025" (Month Day, Year)
   - ✅ CORRECT: "Nov 6, 2025"
   - ❌ WRONG: "Dec 2025" (missing day)
   - ❌ WRONG: "December 2025" (missing day)
   - ❌ WRONG: "2025-12-08" (ISO format for display)
...
# Create article list with dates
articles = []
...
for article in articles:
    # "Dec 08, 2025" then remove leading zero -> "Dec 8, 2025"
    display_date = article['date'].strftime('%b %d, %Y').replace(' 0', ' ')
    print(f"### {article['title']} ({display_date})")
```

### Validation Checklist

Before finalizing WHATS-NEW.md:
- [ ] All article dates are complete (Month Day, Year)
- [ ] NO incomplete dates like "Dec 2025"
